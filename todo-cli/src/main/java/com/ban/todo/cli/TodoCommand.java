...
import com.ban.todo.AddTodoItemParameter;
...
public class TodoCommand {
    ...
    public int add(@CommandLine.Parameters(index = "0") final String item) {
        ...
        final TodoItem todoItem = this.service.addTodoItem(AddTodoItemParameter.of(item));
    ...
    public int markAsDone(@CommandLine.Parameters(index = "0") final int index) {
        ...
        final Optional<TodoItem> item = this.service.markTodoItemDone(index);
        if (!item.isPresent()) {
    ...
    public int list(@CommandLine.Option(names = "--all") final boolean all) {
        final List<TodoItem> items = this.service.listAllTodoItem(all);

        items.stream().map(this::formatItem).forEach(System.out::println);

        if (all) {
            final long doneCount = items.stream().filter(TodoItem::isDone).count();
    ...
    private String formatTotal(final int size, final long doneCount) {
        return "Total: " + size + unit(size) + ", " + doneCount + unit(doneCount);
    }
